...
private let garminHeartRateServiceId = CBUUID(string: "180D")
...
private struct GarminHeartRateMeasurement {
    var heartRate: UInt16

    init(value: Data) throws {
        let reader = ByteReader(data: value)
        let flags = try reader.readUInt8()
        if flags.isBitSet(index: 0) {
...
extension GarminDevice: CBPeripheralDelegate {
    ...
    func peripheral(
        ...
        for characteristic in service.characteristics ?? [] {
            ...
            case garminHeartRateMeasurementCharacteristicId:
    ...
    private func handleHeartRateMeasurement(value: Data) throws {
        let measurement = try GarminHeartRateMeasurement(value: value)
