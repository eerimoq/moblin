...
class VoiceCommandRecognizer: NSObject {
    ...
    func startRecording(onTextUpdated: @escaping (String) -> Void, completion: @escaping (Result<String, Error>) -> Void) {
        stopRecording()
        lastTranscribedText = ""
        ...
        SFSpeechRecognizer.requestAuthorization { authStatus in
            DispatchQueue.main.async {
                guard authStatus == .authorized else {
                    completion(.failure(NSError(domain: "VoiceCommandRecognizer", code: 401, userInfo: [NSLocalizedDescriptionKey: "Permissão de reconhecimento de voz negada"])))
                    return
                }
                
                self.recognitionRequest = SFSpeechAudioBufferRecognitionRequest()
                guard let recognitionRequest = self.recognitionRequest else {
                    completion(.failure(NSError(domain: "VoiceCommandRecognizer", code: 500, userInfo: [NSLocalizedDescriptionKey: "Falha ao criar request"])))
                    return
                }
                ...
                self.recognitionTask = self.speechRecognizer?.recognitionTask(with: recognitionRequest) { [weak self] result, error in
                    ...
                    if let error = error {
                        ...
                        if nsError.code != 301 && nsError.code != 4 {
                            ...
                            completion(.success(self.lastTranscribedText))
                        }
                    }
                }
            }
