...
enum GeminiActionType: String {
    ...
    case displayYouTubeLive = "displayYouTubeLive"
    case removeYouTubeLive = "removeYouTubeLive"
        ...
        for msg in history {
            if let last = sanitized.last {
                if last.role == msg.role {
                    ...
                    let combinedText = (last.parts.compactMap { $0.text } + msg.parts.compactMap { $0.text })
                        .joined(separator: "\n")
                    sanitized[sanitized.count - 1] = ChatMessage(role: last.role, parts: [.init(text: combinedText)])
                } else {
                ...
                functionDeclarations: [
                    ...
                    .init(
                        ...
                        parameters: .init(
                            type: "OBJECT",
                            properties: [
                                "url": .init(type: "STRING", description: "URL completa do YouTube ou página da web (ex: https://www.youtube.com/watch?v=VIDEO_ID ou link do site/mapa)"),
                                "position": .init(type: "STRING", description: "Posição do overlay na tela (opções: top-left, top-right, bottom-left, bottom-right, center, fullscreen)"),
                                "width": .init(type: "INTEGER", description: "Largura em pixels (padrão 600)"),
                                "height": .init(type: "INTEGER", description: "Altura em pixels (padrão 400)"),
                                "opacity": .init(type: "NUMBER", description: "Opacidade de 0.0 a 1.0 (padrão 1.0)")
                            ],
                            required: ["url"]
                        )
                    ),
                    ...
                    .init(
                        ...
                        parameters: .init(
                            type: "OBJECT",
                            properties: [
                                "query": .init(type: "STRING", description: "O termo a ser pesquisado (ex: 'gato fofo', 'live rio de janeiro youtube').")
                            ],
                            required: ["query"]
                        )
                    )
