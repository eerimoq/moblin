...
extension VNFaceObservation {
    ...
    func isLeftEyeOpen() -> Double {
        return isEyeOpen(eye: landmarks?.leftEye)
    }

    func isRightEyeOpen() -> Double {
        return isEyeOpen(eye: landmarks?.rightEye)
    }

    //     1   2
    // 0           3
    //     5   4
    private func isEyeOpen(eye: VNFaceLandmarkRegion2D?) -> Double {
