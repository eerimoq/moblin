...
extension Model {
    ...
    private func handleAction(action: String, value: String?) {
        guard let widget = database.widgets.first(where: { $0.type == .scoreboard }) else { return }
        ...
        if action == "toggle-clock" {
            ...
            // If counting down, reset current time to new duration
            ...
            // Manual edit of the clock numbers
    ...
    func handleExternalScoreboardUpdate(config: SBMatchConfig) {
        ...
        for widget in database.widgets where widget.type == .scoreboard {
            ...
            case "sideBySide": sb.layout = .sideBySide
            case "stackhistory": sb.layout = .stackhistory
            case "stackedInline": sb.layout = .stackedInline
            default: sb.layout = .stacked
            }
            ...
            if let h = Int(config.team1.primaryScore) { sb.generic.score.home = h }
            if let a = Int(config.team2.primaryScore) { sb.generic.score.away = a }
            ...
            // Manual clock edits sent via update-match
    ...
    func getCurrentConfigForEffect() -> SBMatchConfig { return getCurrentConfig() }
    ...
    private func getCurrentConfig() -> SBMatchConfig {
        ...
        if let current = sb?.config, current.matchId == activeId {
            ...
            return SBMatchConfig(matchId: "error", layout: "stacked", team1: SBTeam(name: "FILE MISSING", bgColor: "#000", textColor: "#fff", possession: false, primaryScore: "0", secondaryScore: "0", stat1: "", stat1Label: "", stat2: "", stat2Label: "", stat3: "", stat3Label: "", stat4: "", stat4Label: ""), team2: SBTeam(name: "ERROR", bgColor: "#000", textColor: "#fff", possession: false, primaryScore: "0", secondaryScore: "0", stat1: "", stat1Label: "", stat2: "", stat2Label: "", stat3: "", stat3Label: "", stat4: "", stat4Label: ""), global: SBGlobalStats(title: "ERROR", timer: "00:00", timerDirection: "up", period: "1", periodLabel: "SET", subPeriod: "", primaryScoreResetOnPeriod: false, secondaryScoreResetOnPeriod: false), controls: [:])
        }
        
        if let sb = sb {
            switch sb.layout {
            case .sideBySide: liveConfig.layout = "sideBySide"
            case .stackhistory: liveConfig.layout = "stackhistory"
            case .stackedInline: liveConfig.layout = "stackedInline"
            default: liveConfig.layout = "stacked"
            }
            ...
            if !sb.generic.period.isEmpty { liveConfig.global.period = sb.generic.period }
    ...
    func getAvailableSports() -> [String] {
        ...
        if let files = try? fileManager.contentsOfDirectory(atPath: resourcePath) {
            ...
            for file in jsonFiles {
                ...
                if ["volleyball", "basketball", "hockey", "soccer", "football", "tennis", "generic", "generic_sets"].contains(name) || !sports.isEmpty {
                     sports.insert(name)
        ...
        // Custom Sort: generic, generic_sets, then alphabetical
    ...
    func broadcastStreamStats() {
        let stats = SBStreamStats(battery: "\(Int(battery.level * 100))%", system: systemMonitor.format(), bitrate: bitrate.speedMbpsOneDecimal, bonding: bonding.statistics, rtts: bonding.rtts, uptime: streamUptime.uptime)
