...
extension Model {
        ...
        voiceRecognizer.startRecording(
            ...
            completion: { [weak self] result in
                ...
                case let .failure(error):
                    ...
                    makeToast(title: String(localized: "Transcription failed"))
                    stopGeminiListening()
                }
            }
        )
    ...
    func sendToGemini(text: String) {
        ...
        let sysInstruction = database.gemini.systemInstruction.isEmpty ? nil : database.gemini
            .systemInstruction
        ...
        for action in actions {
            ...
            case .changeScene:
                if let sceneName = action.parameters["sceneName"] as? String {
                    if isRemote {
                        if let remoteScene = remoteControl.settings?.scenes
                            .first(where: { $0.name.lowercased() == sceneName.lowercased() })
                        {
            ...
            case .muteMicrophone:
                if let mute = action.parameters["mute"] as? Bool {
                    if isRemote {
                        ...
                        let status = mute ? String(localized: "Sent: Mute remote mic") :
                            String(localized: "Sent: Unmute remote mic")
                        makeToast(title: status)
                    } else {
                        setMuted(value: mute)
                        let status = mute ? String(localized: "Microphone muted") :
                            String(localized: "Microphone unmuted")
                        makeToast(title: status)
                    }
                }
            case .toggleTorch:
                if let on = action.parameters["on"] as? Bool {
                    if isRemote {
                        remoteControlAssistant?.setTorch(on: on) {
                            let status = on ? String(localized: "Sent: Turn remote torch on") :
                                String(localized: "Sent: Turn remote torch off")
                        ...
                        let status = on ? String(localized: "Torch turned on") :
                            String(localized: "Torch turned off")
                        makeToast(title: status)
                    }
                }
                        ...
                        searchYouTubeLive(query: query) { [weak self] foundUrl in
                            DispatchQueue.main.async {
                                if let foundUrl {
                                    ...
                                    self?
                                        .makeToast(
                                            title: String(localized: "Nenhuma live encontrada para \(query)")
                                        )
                                }
                            }
                        }
            ...
            await geminiService.executeCommand(
                ...
                DispatchQueue.main.async {
                    ...
                    case let .failure(error):
                        ...
                        self
                            .makeToast(
                                title: String(localized: "Erro silencioso: \(error.localizedDescription)")
                            )
                    }
                }
            }
                    ...
                    for match in matches.prefix(5) {
                           ...
                           let titleRange = Range(match.range(at: 2), in: html)
                        {
                            ...
                            if rawUrl.contains("uddg=") {
                                if let queryItems =
                                    URLComponents(string: "https://html.duckduckgo.com" + rawUrl)?.queryItems,
