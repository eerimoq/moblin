...
class RistServerClient {
    ...
    private func handleAudioSampleBuffer(_ sampleBuffer: CMSampleBuffer) {
        ...
        if let error {
            logger.info("rist-server-client: Audio error \(error)")
            return
        }
    ...
    private func handleAudioFormatDescription(_ formatDescription: CMFormatDescription) {
        ...
        guard let audioFormat = AVAudioFormat(streamDescription: streamBasicDescription) else {
            logger.info("rist-server-client: Failed to create audio format")
            audioBuffer = nil
            audioDecoder = nil
            return
        }
        ...
        guard let pcmAudioFormat else {
            logger.info("rist-server-client: Failed to create PCM audio format")
            return
        }
        logger.info("rist-server-client: in: \(audioFormat), out: \(pcmAudioFormat)")
        audioDecoder = AVAudioConverter(from: audioFormat, to: pcmAudioFormat)
        if audioDecoder == nil {
            logger.info("rist-server-client: Failed to create audio decdoer")
        }
    }
    ...
    private func handleVideoFormatDescription(_ formatDescription: CMFormatDescription) {
        ...
        logger.info("rist-server-client: Got new video dimensions \(dimentions)")
...
extension RistServerClient: VideoDecoderDelegate {
