...
extension RistServerClient: MpegTsReaderDelegate {
    func mpegTsReaderAudioBuffer(_ sampleBuffer: CMSampleBuffer) {
        server?.delegate?.ristServerOnAudioBuffer(port: virtualDestinationPort, sampleBuffer)
    }

    func mpegTsReaderVideoBuffer(_ sampleBuffer: CMSampleBuffer) {
        server?.delegate?.ristServerOnVideoBuffer(port: virtualDestinationPort, sampleBuffer)
    }

    func mpegTsReaderSetTargetLatencies(_ videoTargetLatency: Double, _ audioTargetLatency: Double) {
        server?.delegate?.ristServerSetTargetLatencies(port: virtualDestinationPort,
                                                       videoTargetLatency,
                                                       audioTargetLatency)
    }
}
