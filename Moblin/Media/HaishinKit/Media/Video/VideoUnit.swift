...
final class VideoUnit: NSObject {
    ...
    private func findVideoFormat(
        ...
        if isLandscapeStreamAndPortraitUi {
            let formatsWithDynamicAspectRatios = formats.filter { !$0.supportedDynamicAspectRatios.isEmpty }
            if !formatsWithDynamicAspectRatios.isEmpty {
                formats = formatsWithDynamicAspectRatios
