...
final class VideoUnit: NSObject {
                                    ...
                                    isSceneSwitchTransition: Bool) -> Bool
    {
        ...
        let detectionJobs = prepareDetectionJobs(
            needsDetections(enabledEffects, presentationTimeStamp,
                            { $0.needsFaceDetections($1) }, &previousFaceDetectionTimes),
            needsDetections(enabledEffects, presentationTimeStamp,
                            { $0.needsTextDetections($1) }, &previousTextDetectionTimes),
            sampleBuffer.presentationTimeStamp,
            imageBuffer
        )
    ...
    private func needsDetections(
        _ enabledEffects: [VideoEffect],
        _ presentationTimeStamp: Double,
        _ getMode: (VideoEffect, Double) -> VideoEffectDetectionsMode,
        _ previousDetectionTimes: inout [UUID: Double]
    ) -> Set<UUID> {
        var detectionsIntervals: [UUID: Double] = [:]
        var ids: Set<UUID> = []
        for effect in enabledEffects {
            switch getMode(effect, presentationTimeStamp) {
            case .off:
                break
            case let .now(videoSourceId):
                let videoSourceId = videoSourceId ?? sceneVideoSourceId
                ids.insert(videoSourceId)
                previousDetectionTimes[videoSourceId] = presentationTimeStamp
        ...
        for (videoSourceId, interval) in detectionsIntervals {
            if let previousPresentationTimeStamp = previousDetectionTimes[videoSourceId] {
