...
struct AudioSpecificConfig: Equatable {
    ...
    init?(formatDescription: CMFormatDescription?) {
        guard
            ...
            let type =
            AudioObjectType(objectID: MPEG4ObjectID(rawValue: Int(streamDescription.mFormatFlags))),
            let frequency = SamplingFrequency(sampleRate: streamDescription.mSampleRate),
            let channelConfig = ChannelConfiguration(channelCount: streamDescription.mChannelsPerFrame)
        else {
            return nil
        }
        self.type = type
